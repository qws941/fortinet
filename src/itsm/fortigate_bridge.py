...
class ITSMFortiGateBridge:
    ...
    def _execute_policy_rollback(
        self, fg_client, rollback_step: Dict
    ) -> Dict[str, Any]:
        """실제 정책 롤백 실행"""
        try:
            # 롤백 유형에 따른 처리
            ...
            if rollback_type == "delete_policy":
                ...
                if original_policy:
                    result = fg_client.update_policy(
                        original_policy["id"], original_policy["config"]
                    )
                    if result.get("status") == "success":
            ...
            return {
                "success": False,
                "error": "Rollback type not supported or missing data",
            }
